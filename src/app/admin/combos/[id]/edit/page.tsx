...
export default function AdminCombosEditPage() {
  ...
  const fetchCombo = useCallback(async () => {
    ...
    setForm({
      ...
      image: c.image || "",
      servesCount: String(c.servesCount),
      isActive: c.isActive,
    });
    setItems(c.items?.map((item: { menuItemId: string; quantity: number }) => ({ menuItemId: item.menuItemId, quantity: item.quantity })) || []);
    setFetching(false);
  }, [id]);
  ...
  if (fetching) {
    return (
      <AdminShell>
        <AdminPageShell>
          <AdminLoadingState rows={6} height="3rem" />
        </AdminPageShell>
      </AdminShell>
    );
  }
  ...
  return (
    <AdminShell>
      <AdminPageShell>
        <AdminSectionTitle title="Edit Combo Meal" description="Update combo meal details" />
